...
def update_location_user(message, data):
    ...
    add_or_update_coord(message.chat.id, data['name'], data["sys"]["country"], message.location.latitude,
                        message.location.longitude, None)


def update_location_user_city(message, city_name, country_name, latitude, longitude):
    add_or_update_coord(message.chat.id, city_name, country_name,
